...
structure?"* with a byte comparison. No embeddings, no thresholds.

Two records can describe one structure and differ only by an allowed
symmetry — a ring read from a different starting point, an edge list walked
backwards, a tuple whose slots are interchangeable. Canonicalizing collapses
those to a single representative.

### Cycles

- `canonical_cycle(seq, *, reflect=True, key=None) -> tuple` — Lexicographically
  least rotation. `reflect=True` treats a ring and its mirror as one;
  `reflect=False` keeps chirality where traversal direction is meaningful.
- `anchor_cycle(seq, anchor, *, key=None) -> tuple` — Rotate a distinguished
  element to position 0, preserving direction. Raises `ValueError` if the
  anchor is missing or appears more than once (ambiguous rotation).
- `cycle_digest(seq, *, reflect=True, key=None) -> str` — Domain-separated
  SHA-256 of the canonical form. Usable as a dedup key or shard id component.
- `least_rotation(keys) -> int` — Booth's algorithm, O(n), correct under
  repeated elements.

```python
canonical_cycle(["c", "a", "b"]) == canonical_cycle(["a", "b", "c"])   # True
...
- `apply_permutation(items, perm) -> tuple` — `result[i] is items[perm[i]]`.
- `permutation_closure(perms, n, *, max_order=MAX_GROUP_ORDER) -> tuple[tuple[int, ...], ...]`
  — The group generated by `perms`, identity included.
- `canonical_under(items, perms, *, key=None, close=True, max_order=...) -> tuple`
  — Least representative under a declared symmetry.
- `orbit_digest(items, perms, *, key=None, close=True, max_order=...) -> str` —
  Domain-separated digest of the orbit.

`perms` is a **generating set**, closed under composition before the canonical
form is chosen. This is what makes the guarantee hold: without closure, two
members of one orbit reduce to different representatives with no error raised.
`close=False` skips the closure pass for a group you have already closed
yourself — a footgun otherwise. `MAX_GROUP_ORDER` (100,000) bounds the closure.

```python
rotate, reflect = [1, 2, 3, 0], [3, 2, 1, 0]
...
- They must be **JSON-serializable**. For anything else — `bytes`, `datetime`,
  `set`, a domain object — pass `key=`, which maps each element onto a
  serializable surrogate exactly like `sorted(key=...)`. The returned tuple
  still holds the original elements; digests cover the surrogates.
