...
- **Errors on `resolve*` propagate; they are never swallowed into `None`.** Transport and permission failures (`AccessDenied`, throttling, connection resets) raise `NetworkUnavailable` out of `resolve` / `resolve_sealed` / `resolve_encrypted` / `resolve_manifest`. This diverges deliberately from the IPFS backend, which returns `None` on transport error: in a hosted worker (e.g. an async Lambda), a swallowed error would report success on empty/partial data — a silent data-loss / false-success. `resolve_manifest` additionally content-address-verifies the parsed manifest against the requested key (`IntegrityError` on mismatch), mirroring `resolve()`'s `shard_id` check, since the manifest feeds the receipt/lineage path.
- `pin`/`unpin` are documented no-ops that report success — S3 durability is a bucket-level property (lifecycle/versioning), not a per-object pin, so there is nothing to pin; `unpin` notably does **not** delete (use an explicit S3 delete or lifecycle rule). `ShardLocation.local` is `False` for S3 objects (an L2/remote store), so a local-first resolver does not wrongly skip the network fallback.
